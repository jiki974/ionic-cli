...
function printLibVersions(data) {
  ...
  log.info('Local Ionic version: '.green + data.local.version + '  (' + data.versionFilePath + ')');

  getVersionData('latest').then(function(versionData) {
    log.info('Latest Ionic version: '.green + versionData.version_number +
...
function getVersionData(data, version) {
  ...
  request({ url: url, proxy: proxy }, function(err, res, body) {
    try {
      ...
      log.error('Error loading ' + version + ' version information');
      q.reject();
    }
  });
...
function updateLibVersion(data) {
  ...
  log.info('Are you sure you want to replace '.green + libPath +
           ' with an updated version of Ionic?'.green);

  var promptProperties = {
    areYouSure: {
      name: 'areYouSure',
      description: '(yes/no):'.yellow,
      required: true
    }
  };
...
function getLatest() {
  ...
  getVersionData(version).then(function(versionData) {
    if (version === 'latest') {
      log.info('Latest version: '.green + versionData.version_number +
               '  (released ' + versionData.release_date + ')');
    } else {
      log.info('Version: '.green + versionData.version_number +
               '  (released ' + versionData.release_date + ')');
    }
...
function downloadZip(data, version) {
  ...
  log.info('Downloading: '.green + archivePath);
  ...
  try {
    ...
    writeStream.on('close', function() {
      ...
      try {
        ...
        log.info('Ionic version updated to: '.green + data.versionData.version_number);
