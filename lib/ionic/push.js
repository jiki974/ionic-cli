  ...
  .then(function(jar) {
    ...
    if (argv._[1] && argv._[1].trim() === 'webhook_url') {
      if (argv._[2]) {
        ...
        log.error('You need to specify a webhook url!'.red);
      }
...
function set_webhook_url(task, webhook_url) { // eslint-disable-line camelcase
  ...
  if (!project.get('app_id')) {
    log.error('You need to upload your app first!'.red);
    return false;
  }
...
function send_push() { // eslint-disable-line camelcase
  ...
  promptProperties['push-api-key'] = {
    name: 'push-api-key',
    description: 'Your private API key'.yellow,
    required: true,
    isFile: false
  };
  ...
  promptProperties['device-token'] = {
    name: 'device-token',
    description: 'Device token'.yellow,
    required: true,
    isFile: false
  };
  ...
  promptProperties['alert'] = {
    name: 'alert',
    description: 'Notification alert message'.yellow,
    required: false,
    isFile: false
  };
  ...
  promptProperties['badge'] = {
    name: 'badge',
    description: 'Badge count'.yellow,
    required: false,
    isFile: false
  };
  ...
  promptProperties['sound'] = {
    name: 'sound',
    description: 'Sound file name'.yellow,
    required: false,
    isFile: false
  };
...
function set_production_mode(task) { // eslint-disable-line camelcase
  ...
  if (task.inputValues['production-mode'] === true) {
    log.error('You need to specify a value [y/n] to set the production mode!'.red);
    return false;
  }
...
function upload_cert(task) { // eslint-disable-line camelcase
  ...
  if (argv['ios-dev-cert']) {
    promptProperties['ios-push-cert'] = {
      name: 'ios-push-cert',
      description: 'iOS Dev Push Certificate File (.p12)'.yellow,
      required: true,
      conform: fileExists,
      isFile: true
    };
    ...
    promptProperties['ios-push-cert'] = {
      name: 'ios-push-cert',
      description: 'iOS Prod Push Certificate File (.p12)'.yellow,
      required: true,
      conform: fileExists,
      isFile: true
    };
