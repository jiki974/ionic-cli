...
var settings = {
  ...
  options: {
    '--profile <tag>': '(' + 'credentials <platform>'.yellow +
      ') Specify the profile on which these credentials are saved',
    '--keystore|-s <path>': '(' + 'credentials android'.yellow +
      ') Specify the location of your keystore file',
    '--keystore-password|-p <password>': '(' + 'credentials android'.yellow +
      ') Specify your keystore password (exclude for prompt)',
    '--key-alias|-k <alias>': '(' + 'credentials android'.yellow +
      ') Specify your key alias for this app',
    '--key-password|-w <password>': '(' + 'credentials android'.yellow +
      ') Specify your key password for this app (exclude for prompt)',
    '--cert|-c <path>': '(' + 'credentials ios'.yellow +
      ') Specify the location of your .p12 file',
    '--cert-password|-p <password>': '(' + 'credentials ios'.yellow +
      ') Specify your certificate password (exclude for prompt)',
    '--provisioning-profile|-r <path>': '(' + 'credentials ios'.yellow +
      ...
      if (body.data.length === 0) {
        ...
        _.each(body.data, function(profile) {
          table.push([
            profile.name,
            profile.tag,
            typeof profile.credentials.android === 'undefined' ? '✗'.red : '✓'.green,
            typeof profile.credentials.ios === 'undefined' ? '✗'.red : '✓'.green
          ]);
        });
